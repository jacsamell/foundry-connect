...
const getVanityNumbers = (numbers: string, desiredCount: number) => {
    ...
    const vanityNumbers = predictions
        ...
        .map(prediction => [...numbers].map(number => ['0', '1'].includes(number) ? number : prediction.shift()).join(''));
    ...
    if (vanityNumbers.length < desiredCount && searchNumbers.length > 1) {
        const shorterWords = getVanityNumbers(
            numbers.substr(1), desiredCount - vanityNumbers.length
        );
